    ...
    <div className="flex flex-col h-screen p-4 md:p-8 bg-slate-50 font-sans">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Manage and monitor room availability at a glance.</p>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="relative w-full max-w-5xl h-[90vh] flex flex-col bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in duration-300">
              <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Guest House Management</span>
                  <h2 className="text-xl font-bold text-slate-800">Booking Schedule</h2>
                </div>
                <button 
                  onClick={() => setIsCalendarOpen(false)}
                  className="p-2 hover:bg-red-50 rounded-full transition-all group"
                >
                  <img src={crossIcon} className="w-6 h-6 opacity-40 group-hover:opacity-100 " alt="close" />
                </button>
              </div>
