...
function Carousel(){
    ...
    function handlePrev(){
        carousalIndex == 0 ? setCarousalIndex(roomTypes.length - 1) : setCarousalIndex(carousalIndex-1);
    }

    function handleNext(){
        carousalIndex == roomTypes.length-1 ? setCarousalIndex(0) : setCarousalIndex(carousalIndex+1);
    }

    return(
        <div className={`relative bg-cover bg-center w-full h-64 rounded-xl shadow-xl`}>
            ...
            <button className="h-full w-[3%] cursor-pointer absolute left-1 top-[5%] bg-transparent font-bold text-sm text-white" onClick={handlePrev}>
            ...
            <button className=" h-full w-[3%] cursor-pointer absolute top-[5%] right-1 bg-transparent font-bold text-sm text-white" onClick={handleNext}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-5 h-5">
            ...
            {
                roomTypes.map(( _ ,i)=>(
                    <div className={`h-2 rounded-full transition-all ${carousalIndex==i ? "w-6 bg-white" :"w-2 bg-white/50"}`}>
